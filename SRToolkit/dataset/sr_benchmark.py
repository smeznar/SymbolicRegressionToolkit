...
class SRBenchmark:
    ...
    def create_dataset(self, dataset_name: str) -> SRDataset:
        ...
        if dataset_name in self.datasets:
            ...
            if os.path.exists(self.datasets[dataset_name]["path"]):
                ...
                data = np.load(
                    self.datasets[dataset_name]["path"][:-4], allow_pickle=True
                )
            else:
                raise ValueError(
                    f"Could not find dataset {dataset_name} at {self.datasets[dataset_name]['path']}"
                )
    ...
    def feynman(dataset_directory: str) -> "SRBenchmark":
        ...
        sl_1v.add_symbol(
            "C", "const", 5, np_fn="np.full(X.shape[0], C[{}])", latex_str=r"C_{{{}}}"
        )
    ...
    def nguyen(dataset_directory: str) -> "SRBenchmark":
        ...
        sl_1v.add_symbol(
            "log", symbol_type="fn", precedence=5, np_fn="{} = np.log10({})"
        )
        sl_1v.add_symbol(
            "sqrt", symbol_type="fn", precedence=5, np_fn="{} = np.sqrt({})"
        )
        sl_1v.add_symbol(
            "^2", symbol_type="fn", precedence=5, np_fn="{} = np.power({}, 2)"
        )
        sl_1v.add_symbol(
            "^3", symbol_type="fn", precedence=5, np_fn="{} = np.power({}, 3)"
        )
